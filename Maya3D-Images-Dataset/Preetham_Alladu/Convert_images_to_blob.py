...
for subdir, dirs, files in os.walk(direcotry_name):
    for file in files:
        ...
        cat_img = filepath.replace("C:\\Users\\Preetham Reddy\\Desktop\\meh\\", "")
        ...
        e = re.findall("X\d+.", filepath)
        qewg=e[0].replace("X","").replace("_", "")
        x.append(qewg)

        e = re.findall("Y\d+.", filepath)
        qewg=e[0].replace("Y","").replace("_", "")
        y.append(qewg)

        e = re.findall("Z\d+.", filepath)
...
dataFrame.to_csv('C:\\Users\\Preetham Reddy\\Desktop\\final.csv',encoding='utf-8')
